#!/usr/bin/env python3
"""
Verificación de coincidencia entre el algoritmo CRC del fabricante y el ejemplo del manual GT06
"""
...
print("=== VERIFICACIÓN DE COINCIDENCIA CRC DEL FABRICANTE ===")
print()
...
print("1. VERIFICACIÓN DEL PAQUETE DE LOGIN:")
...
# Verificar CRC del login usando algoritmo del fabricante
...
print(f"CRC esperado (algoritmo fabricante): {expected_crc.hex()}")
print(f"CRC recibido en manual: {error_check.hex()}")
print(f"¿CRC coincide con algoritmo del fabricante?: {'SÍ' if expected_crc == error_check else 'NO'}")

print()
print("2. VERIFICACIÓN DEL ACK:")
...
# Verificar CRC del ACK usando algoritmo del fabricante
...
print(f"ACK CRC esperado (algoritmo fabricante): {expected_ack_crc.hex()}")
print(f"ACK CRC recibido en manual: {ack_error_check.hex()}")
print(f"¿ACK CRC coincide con algoritmo del fabricante?: {'SÍ' if expected_ack_crc == ack_error_check else 'NO'}")

print()
print("3. VERIFICACIÓN DE NUESTRO ACK GENERADO:")
...
print(f"Nuestro CRC (algoritmo fabricante): {our_crc.hex()}")
...
print()
print("4. ANÁLISIS DETALLADO DEL ALGORITMO CRC:")
print("Verificando paso a paso el cálculo CRC...")

# Verificar el cálculo CRC del login paso a paso
print(f"\nPara el login:")
print(f"Datos: {login_data.hex()}")
print(f"Bytes individuales: {' '.join([f'{b:02X}' for b in login_data])}")

# Calcular CRC paso a paso
fcs = 0xFFFF
for i, byte in enumerate(login_data):
    old_fcs = fcs
    fcs = (fcs >> 8) ^ CRC_TAB16[(fcs ^ byte) & 0xFF]
    print(f"Byte {i+1}: {byte:02X}, FCS anterior: {old_fcs:04X}, FCS nuevo: {fcs:04X}")

final_crc = ~fcs & 0xFFFF
print(f"FCS final: {fcs:04X}")
print(f"CRC final (NOT): {final_crc:04X}")
print(f"CRC en bytes BE: {bytes([(final_crc >> 8) & 0xFF, final_crc & 0xFF]).hex()}")
print(f"CRC esperado del manual: {error_check.hex()}")

print()
print("5. CONCLUSIÓN:")
if expected_crc == error_check and expected_ack_crc == ack_error_check:
    print("✅ PERFECTO: El algoritmo CRC del fabricante coincide EXACTAMENTE con el ejemplo del manual")
    print("✅ Nuestro ACK generado es 100% correcto")
else:
    print("❌ PROBLEMA: El algoritmo CRC del fabricante NO coincide con el ejemplo del manual")
    print("❌ Necesitamos revisar el algoritmo CRC")
