#!/usr/bin/env python3
"""
Script para probar diferentes variantes de ACK y determinar cuál hace que el dispositivo envíe posición
"""
...
HOST = '0.0.0.0'
PORT = 5004  # Puerto diferente para pruebas

LOG_FILE = "test_ack_variants.txt"
...
def send_ack_variant(serial, conn, variant="standard"):
    ...
    elif variant == "manual_exact":
        # Exactamente como el manual: 7878 + 05 + 01 + serial + CRC + 0D0A
        ack_data = b'\x05\x01' + serial
        crc = crc16_itu_factory_bytes_be(ack_data)
        ack = b'\x78\x78' + ack_data + crc + b'\x0D\x0A'
...
def handle_login_with_variants(data, conn, conn_data):
    """
    Maneja el login probando diferentes variantes de ACK
    """
    try:
        ...
        def test_variants():
            variants = [
                "manual_exact",  # Exactamente como el manual
            ...
            for variant in variants:
                time.sleep(5)  # Esperar 5 segundos entre variantes
                
                if conn_data.get('position_received', False):
                    log(f"[VARIANT] ¡POSICIÓN RECIBIDA! ACK tipo '{variant}' funcionó")
                    return
                
                log(f"[VARIANT] Probando ACK tipo '{variant}'")
        ...
        variant_thread = threading.Thread(target=test_variants, daemon=True)
        variant_thread.start()
        
        return ack
        
    except Exception as e:
        log(f"[ERROR] Error en handle_login_with_variants: {e}")
        return None

def main():
    log(f"[VARIANT] Servidor de pruebas de variantes ACK iniciado en {HOST}:{PORT}")
    log(f"[VARIANT] Probando diferentes tipos de ACK para encontrar el correcto")
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            try:
                conn, addr = s.accept()
                with conn:
                    log(f"[VARIANT] Conexión entrante desde {addr}")
                    ...
                    while True:
                        ...
                        log(f"[VARIANT] [RECIBIDO] {data.hex()}")
                        
                        if len(data) < 8:
                            log("[VARIANT] [ERROR] Paquete demasiado corto")
                            continue
                        ...
                        if data.startswith(b'\x78\x78'):
                            if len(data) >= 4:
                                ...
                                log(f"[VARIANT] Protocol: 0x{protocol:02X}, Length: {packet_length}")
                                
                                if protocol == 0x01:  # Login
                                    log("[VARIANT] Procesando login...")
                                    ack = handle_login_with_variants(data, conn, conn_data)
                                    if ack:
                                        log("[VARIANT] ✓ ACK de login enviado correctamente")
                                    else:
                                        log("[VARIANT] ✗ Error enviando ACK de login")
                                    
                                elif protocol == 0x12:  # Posición
                                    log("[VARIANT] ¡POSICIÓN RECIBIDA! ACK funcionó")
                                    conn_data['position_received'] = True
                                    
                                    # Enviar ACK para posición
                                    if len(data) >= 25:
                                        pos_serial = data[23:25]
                                        send_ack_variant(pos_serial, conn, "standard")
                                        log("[VARIANT] ✓ ACK de posición enviado")
                                    
                                elif protocol == 0x13:  # Estado
                                    log("[VARIANT] Estado recibido - ACK aún no reconocido")
                                    # Enviar ACK para estado
                                    if len(data) >= 8:
                                        status_info = data[4:-4]
                                        if len(status_info) >= 7:
                                            status_serial = status_info[-2:]
                                            send_ack_variant(status_serial, conn, "standard")
                                            log("[VARIANT] ACK para estado enviado")
                                    
                                elif protocol == 0x23:  # Heartbeat
                                    log("[VARIANT] Heartbeat recibido")
                                    
                                elif protocol == 0x26:  # Alarma
                                    log("[VARIANT] Alarma recibida")
                                    
                                else:
                                    log(f"[VARIANT] Protocolo desconocido: 0x{protocol:02X}")
                        else:
                            log("[VARIANT] [ERROR] Paquete no comienza con 7878")
                            
            except socket.error as e:
                log(f"[VARIANT] [ERROR] Error de socket: {e}")
            except Exception as e:
                log(f"[VARIANT] [ERROR] Error inesperado: {e}")
